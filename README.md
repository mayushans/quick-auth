...
│   ├── app-server.conf     # 应用配置示例
│   └── snippets/
│       ├── auth-location.conf # auth_request 端点代理文件
│       └── auth-use.conf      # 引入访问控制文件
...
server {
    ...
    include snippets/auth-location.conf;

    # 受保护的资源 
    location /private/ {
        # 首先进行鉴权，验证用户是否已登录
        include snippets/auth-use.conf;
